...
export function Layout() {
  ...
  return (
    <div className="w-full h-full flex flex-col scanlines text-foreground bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-[#0a0a0f] via-[#050508] to-black relative">
      {/* Top System Bar */}
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <button
                ...
                className={`relative group w-14 h-14 flex items-center justify-center transition-all duration-300 clip-edges ${
                  isActive ? "bg-primary/20 text-primary neon-border" : "text-muted-foreground hover:text-primary hover:bg-primary/10"
                }`}
                title={item.label}
              >
                ...
                {isActive && (
                  <motion.div 
                    layoutId="activeTab"
                    className="absolute inset-0 border-2 border-primary clip-edges"
