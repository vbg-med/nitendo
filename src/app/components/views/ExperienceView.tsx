  ...
  return (
    <div className="h-full flex flex-col gap-6">
      ...
      <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar relative">
        ...
        <div className="absolute left-4 md:left-[15%] top-0 bottom-0 w-px bg-primary/20">
          <div className="absolute top-0 bottom-0 w-full bg-gradient-to-b from-primary via-secondary to-primary/20 opacity-50"></div>
          {/* Animated glow on the line */}
          <motion.div 
            className="absolute top-0 left-[-1px] w-[3px] h-32 bg-primary shadow-[0_0_15px_#00f3ff]"
            animate={{ top: ["0%", "100%"] }}
        ...
        <div className="space-y-12 pb-12 pt-4 relative">
          {TIMELINE.map((item, idx) => (
            ...
            >
              {/* Timeline Node */}
              <div className="absolute left-4 md:left-[15%] w-4 h-4 rounded-none bg-black border-2 border-primary transform -translate-x-[7px] md:-translate-x-[7px] mt-1.5 md:mt-0 z-10 group-hover:scale-150 transition-transform group-hover:bg-primary group-hover:shadow-[0_0_10px_#00f3ff] rotate-45"></div>
              
              {/* Left Side - Year & Level */}
              <div className="ml-12 md:ml-0 md:w-[15%] md:text-right pt-1 shrink-0">
                <div className="text-primary font-mono text-sm group-hover:text-white transition-colors">{item.year}</div>
                <div className="text-xs text-muted-foreground font-mono mt-1">{item.level}</div>
              </div>
              
              {/* Right Side - Content Card */}
              <HoloCard className="ml-12 md:ml-0 md:flex-1 hover:border-primary/50 transition-colors">
                ...
                <div className="flex flex-wrap gap-2">
                  {item.tech.map((t, i) => (
                    <span key={i} className="px-2 py-0.5 bg-black/50 border border-white/10 text-xs font-mono text-gray-300">
                      {t}
                    </span>
                  ))}
                </div>
          ...
          <div className="relative flex flex-col md:flex-row gap-6 md:gap-8 opacity-50">
             <div className="absolute left-4 md:left-[15%] w-4 h-4 rounded-none bg-black border-2 border-white/30 transform -translate-x-[7px] md:-translate-x-[7px] rotate-45"></div>
             <div className="ml-12 md:ml-0 md:w-[15%] md:text-right pt-1 shrink-0">
