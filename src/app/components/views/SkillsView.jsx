...
import { HoloCard, ProgressBar } from "../ui/CyberUI";
      ...
      <div className="flex flex-col md:flex-row gap-6 h-full min-h-0 overflow-hidden pb-4">
          ...
          <div className="w-full aspect-square max-h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="70%" data={radarData}>
                ...
                <Radar name="Skills" dataKey="A" stroke="#00f3ff" fill="#00f3ff" fillOpacity={0.4}/>
                <Tooltip contentStyle={{ backgroundColor: 'rgba(0,0,0,0.8)', border: '1px solid #00f3ff', fontFamily: 'monospace', fontSize: '12px' }} itemStyle={{ color: '#00f3ff' }}/>
              </RadarChart>
            </ResponsiveContainer>
          </div>
        ...
        <div className="w-full md:w-2/3 flex flex-col gap-4 overflow-hidden">
          ...
          <div className="flex gap-2 shrink-0 overflow-x-auto pb-2 scrollbar-none">
            ...
            return (<button key={cat.id} onClick={() => setActiveTab(cat.id)} className={`flex items-center gap-2 px-4 py-2 font-['Orbitron'] text-sm tracking-wider border transition-all whitespace-nowrap ${isActive
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-4">
            {SKILLS[activeTab].map((skill, idx) => (<motion.div key={skill.name} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.1 }} className="bg-black/40 border border-primary/20 p-4 relative group hover:border-primary/60 transition-colors">
                ...
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className={`text-[10px] font-mono px-1 border ${skill.status === 'EQUIPPED' ? 'text-primary border-primary/50' :
