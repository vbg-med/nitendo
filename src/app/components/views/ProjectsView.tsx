      ...
      <div className="flex-1 relative flex items-center justify-center min-h-0">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="w-full max-w-4xl h-full flex flex-col md:flex-row glass-panel border border-primary/30 overflow-hidden shadow-[0_0_30px_rgba(0,0,0,0.8)] relative group"
          >
            {/* Holographic scanning overlay */}
            <div className="absolute inset-0 pointer-events-none overflow-hidden z-20">
              <motion.div 
                className="w-full h-8 bg-gradient-to-b from-transparent via-primary/10 to-transparent"
                animate={{ top: ["-10%", "110%"] }}
            ...
            <div className="w-full md:w-1/2 relative min-h-[200px] md:min-h-full border-r border-primary/20">
              <ImageWithFallback 
                src={current.image} 
                alt={current.title} 
                className="absolute inset-0 w-full h-full object-cover opacity-60 mix-blend-luminosity filter contrast-125 group-hover:opacity-80 transition-opacity"
              />
              ...
              <div className="absolute top-4 left-4 flex flex-col gap-2 z-10">
                ...
                <span className={`bg-black/80 border px-2 py-1 text-[10px] font-mono flex items-center gap-2 w-fit ${diffColors[current.difficulty as keyof typeof diffColors]}`}>
              ...
              <div className="mb-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {current.tech.map((t, i) => (
                    <span key={i} className="px-2 py-1 border border-white/20 bg-white/5 text-xs font-mono text-white hover:border-primary hover:text-primary transition-colors cursor-default">
                      {t}
                    </span>
                  ))}
                </div>
          ...
          {MISSIONS.map((_, idx) => (
            <button
              ...
              className={`w-12 h-1 transition-all duration-300 ${
                idx === currentIndex ? "bg-primary shadow-[0_0_10px_#00f3ff]" : "bg-white/20 hover:bg-white/40"
              }`}
            />
          ))}
        </div>
