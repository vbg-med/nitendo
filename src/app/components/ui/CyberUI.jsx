...
export const GlitchText = ({ text, className = "", as: Component = "h1" }) => {
    return (<Component className={`relative inline-block font-['Orbitron'] ${className}`} data-text={text}>
...
export const NeonButton = ({ children, onClick, className = "", variant = "primary" }) => {
    ...
    return (<button onClick={onClick} className={`relative px-6 py-2 border clip-edges transition-all duration-300 font-['Orbitron'] font-bold tracking-widest text-sm uppercase cursor-pointer ${color} ${shadow} ${className}`}>
...
export const HoloCard = ({ children, className = "" }) => {
    return (<div className={`relative glass-panel p-1 group ${className}`}>
...
export const ProgressBar = ({ label, percentage, color = "primary" }) => {
    const bgColors = {
        primary: "bg-primary",
    ...
    return (<div className="mb-4">
      ...
      <div className="h-2 w-full bg-black/60 border border-white/10 relative overflow-hidden clip-edges">
        <motion.div initial={{ width: 0 }} animate={{ width: `${percentage}%` }} transition={{ duration: 1, ease: "easeOut" }} className={`h-full ${bgColors[color]} relative`}>
...
export const StatBox = ({ label, value, icon: Icon, trend }) => {
    return (<div className="border border-primary/20 bg-primary/5 p-3 flex items-center gap-3 relative overflow-hidden group">
